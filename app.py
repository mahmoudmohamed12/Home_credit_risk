...
TRAIN_DATA_PATH = BASE_DIR / "app_train_cleaned.parquet"
...
# =============================================================================
# EDA — Real data loader (expects application_train.csv in project root)
# =============================================================================
@st.cache_data(show_spinner="Loading training data...")
def load_training_data(path: Path) -> pd.DataFrame | None:
    """Load application_train.csv if it exists in the project directory."""
    if not path.exists():
        return None
    try:
        df = pd.read_parquet(path)
        if "TARGET" not in df.columns:
            st.warning("Training file loaded but no TARGET column found. EDA will be limited.")
        return df
    except Exception as exc:
        st.error(f"Failed to load training data from `{path}`: {exc}")
        return None


def _prepare_train_df(df: pd.DataFrame) -> pd.DataFrame:
    """Derive helper columns needed for plotting."""
    df = df.copy()
    if "DAYS_BIRTH" in df.columns and "YEARS_BIRTH" not in df.columns:
        df["YEARS_BIRTH"] = (df["DAYS_BIRTH"].abs() / 365.25).astype(int)
    # Derive EXT_SOURCE_MEAN if components exist
    ext_cols = [c for c in ["EXT_SOURCE_1", "EXT_SOURCE_2", "EXT_SOURCE_3"] if c in df.columns]
    if ext_cols and "EXT_SOURCE_MEAN" not in df.columns:
        df["EXT_SOURCE_MEAN"] = df[ext_cols].mean(axis=1)
    # Derive CREDIT_INCOME_RATIO if components exist
    if "AMT_CREDIT" in df.columns and "AMT_INCOME_TOTAL" in df.columns and "CREDIT_INCOME_RATIO" not in df.columns:
...
def plot_age_distribution(df: pd.DataFrame) -> go.Figure:
    ...
    if target_col:
        for target_val, color, name in [(0, "#1D9E75", "Repaid"), (1, "#D85A30", "Defaulted")]:
            subset = df.loc[df[target_col] == target_val, "YEARS_BIRTH"]
        ...
        fig.add_trace(go.Histogram(
            x=df["YEARS_BIRTH"], histnorm="probability density", nbinsx=40,
...
def plot_ext_source_violin(df: pd.DataFrame, source: str = "EXT_SOURCE_2") -> go.Figure:
    target_col = "TARGET" if "TARGET" in df.columns else None
    fig = go.Figure()
    if target_col and source in df.columns:
        for target_val, color, name in [(0, "#1D9E75", "Repaid"), (1, "#D85A30", "Defaulted")]:
            subset = df[df[target_col] == target_val][source].dropna()
        ...
        subset = df[source].dropna()
        fig.add_trace(go.Violin(
            x=[source] * len(subset), y=subset,
            box_visible=True, meanline_visible=True,
            fillcolor="#3b82f6", opacity=0.7, line_color="white",
        ))
    fig.update_layout(
        title=f"{source} Distribution — Strongest Single Predictor",
        yaxis_title=f"{source} score", height=420,
    )
    return fig
...
def plot_age_band_default_rate(df: pd.DataFrame) -> go.Figure:
    target_col = "TARGET" if "TARGET" in df.columns else None
    if "YEARS_BIRTH" not in df.columns:
        return go.Figure().update_layout(title="YEARS_BIRTH not available")

    df["AGE_BAND"] = pd.cut(df["YEARS_BIRTH"], bins=range(18, 76, 5))
    if target_col:
        age_risk = (df.groupby("AGE_BAND", observed=True)[target_col]
                      .agg(["mean", "count"])
                      .rename(columns={"mean": "default_rate", "count": "applicants"})
                      .reset_index())
        age_risk["default_rate_pct"] = (age_risk["default_rate"] * 100).round(2)
    else:
        age_risk = (df.groupby("AGE_BAND", observed=True)
                      .size()
                      .reset_index(name="applicants"))
        age_risk["default_rate_pct"] = 0.0
...
def plot_cir_default_rate(df: pd.DataFrame) -> go.Figure:
    ...
    if "CREDIT_INCOME_RATIO" not in df.columns:
        return go.Figure().update_layout(title="CREDIT_INCOME_RATIO not available")
    ...
    if target_col:
        cir_risk = (df.groupby("CIR_BAND", observed=True)[target_col]
                      .mean().mul(100).round(2).reset_index())
        cir_risk.columns = ["CIR_BAND", "default_rate_pct"]
    else:
        cir_risk = (df.groupby("CIR_BAND", observed=True)
                      .size()
                      .reset_index(name="count"))
        cir_risk["default_rate_pct"] = 0.0
        cir_risk.columns = ["CIR_BAND", "default_rate_pct"]
...
def plot_credit_income_scatter(df: pd.DataFrame) -> go.Figure:
    target_col = "TARGET" if "TARGET" in df.columns else None
    if "CREDIT_INCOME_RATIO" not in df.columns or "EXT_SOURCE_2" not in df.columns:
        return go.Figure().update_layout(title="Required columns not available")

    sample = df.sample(min(5000, len(df)), random_state=42)
    color_kw = {"color": target_col, "color_discrete_map": {0: "#1D9E75", 1: "#D85A30"}} if target_col else {}
    fig = px.scatter(
        ...
        labels={target_col: "Default"} if target_col else {},
        **color_kw,
    )
...
def plot_feature_violin(df: pd.DataFrame, feature: str, title_suffix: str = "") -> go.Figure:
    target_col = "TARGET" if "TARGET" in df.columns else None
    fig = go.Figure()
    if target_col and feature in df.columns:
        for target_val, color, name in [(0, "#1D9E75", "Repaid"), (1, "#D85A30", "Defaulted")]:
        ...
        fig.add_trace(go.Violin(
            x=[feature] * len(subset), y=subset,
            box_visible=True, meanline_visible=True,
            fillcolor="#3b82f6", opacity=0.7, line_color="white",
        ))
    else:
        return go.Figure().update_layout(title=f"{feature} not available")

    fig.update_layout(
        title=f"{feature} {title_suffix}".strip(),
        yaxis_title=feature, height=350,
...
def render_eda_tab() -> None:
    ...
    if df is None:
        st.warning(
            f"**Real training data not found.**  \n"
            f"Expected file: `{TRAIN_DATA_PATH}`  \n\n"
            f"To enable real EDA plots, place your `application_train.csv` (or any CSV with a `TARGET` column) "
            f"in the same folder as this app. The file is too large to upload here, so it must be added locally."
        )
        st.info(
            ...
            "project/\n"
            "├── app.py\n"
            "├── application_train.csv   <-- add this file\n"
            "└── model/\n"
            "    ├── final_model.pkl\n"
    ...
    st.success(f"Loaded real training data: **{len(df):,} rows**, **{len(df.columns)} columns**.")
    df = _prepare_train_df(df)
    ...
    with c2:
        st.plotly_chart(plot_ext_source_violin(df, "EXT_SOURCE_2"), use_container_width=True)
    ...
    with c6:
        if "EXT_SOURCE_MEAN" in df.columns and "TARGET" in df.columns:
            ...
            st.info("EXT_SOURCE_MEAN or TARGET not available for summary table.")

    # Row 4: Key feature violins from selected_features.json
    st.markdown("---")
    st.subheader("Top Risk-Signal Distributions")
    feature_configs = [
        ("EXT_SOURCE_MEAN", "— lower values indicate higher risk"),
        ("CREDIT_INCOME_RATIO", "— higher ratios increase default risk"),
        ("PREV_WAS_REFUSED_MEAN", "— prior refusals signal risk"),
        ("BUREAU_DAYS_CREDIT_MEAN", "— longer credit history protects"),
    ...
    cols = st.columns(2)
    for idx, (feature, interpretation) in enumerate(feature_configs):
        with cols[idx % 2]:
            st.plotly_chart(
                plot_feature_violin(df, feature, interpretation),
